...
export async function PATCH(req: Request, { params }: { params: { id: string } }) {
  ...
  const [updated] = await db
    .update(pemasukanLain)
    .set(data)
    .where(eq(pemasukanLain.id, params.id))
    .returning();
  if (!updated) return NextResponse.json({ error: "Tidak ditemukan" }, { status: 404 });
  return NextResponse.json({ pemasukanLain: updated });
}
...
export async function DELETE(_req: Request, { params }: { params: { id: string } }) {
  ...
  await db.delete(pemasukanLain).where(eq(pemasukanLain.id, params.id));
